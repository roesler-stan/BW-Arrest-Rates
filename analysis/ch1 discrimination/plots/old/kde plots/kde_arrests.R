...
plots_list <- lapply(seq_along(subsets_list), function(i) {
  plot_data <- ddply(subsets_list[[i]], .(ori), summarise, arrested = sum(arrested, na.rm = T))
  
  p <- ggplot(plot_data, aes(arrested)) + geom_density(alpha = 2/3, adjust = 1/5) + theme_bw() +
    ggtitle(names(subsets_list)[[i]]) + xlab('Number of Arrested Offenders') + ylab('Agency Density') +
    scale_x_log10()
  p
})
...
g <- arrangeGrob(plots_grid, top = textGrob('Number of Arrested Offenders'))
...
ggsave("kde_arrested.png", plot = g2, dpi = 300, width = 12, height = 10)
