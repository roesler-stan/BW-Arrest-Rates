...
all_cols <- c("arrested", "ori", "black_not_white", "w_officers_percent",
              "victim_analyzing_black_not_white", "victim_analyzing_known")
...
multilevel_wvictims <- lapply(seq_along(subsets_wvictims), function(i) {
  ...
  model1 <- glmer(arrested ~ black_not_white + w_officers_percent + victim_analyzing_black_not_white +
                    victim_analyzing_known + (1 | ori), data = subset_data, family = binomial)
  model2 <- glmer(arrested ~ black_not_white * w_officers_percent + victim_analyzing_black_not_white +
                    victim_analyzing_known + (1 | ori), data = subset_data, family = binomial)
