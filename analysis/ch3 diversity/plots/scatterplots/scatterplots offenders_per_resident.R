...
plots_list <- lapply(seq_along(subsets_list), function(i) {
  zipcode_offenders_per_resident_arrested <- ddply(subsets_list[[i]], .(zipcode_offenders_per_resident, black_not_white),
                                      summarise, mean_arrested = mean(arrested) * 100,
                                      sd_arrested = sd(arrested),
                                      offenders_count = sum(!is.na(arrested)))
  
  p <- ggplot(zipcode_offenders_per_resident_arrested, aes(x = zipcode_offenders_per_resident, y = mean_arrested,
                                              color = factor(black_not_white),
                                              size = offenders_count)) +
    geom_point(alpha = 7/10) + scale_size(range = c(1, 3), guide = 'none') +
    stat_smooth(method = "loess") +
    xlab("Zip Codes' Offenders per Resident") + ylab("Zip Codes' % Offenders Arrested") +
...
ggsave("offenders_per_resident_percent_arrested.png", plot = plots_grid,
...
raw_plots_list <- lapply(seq_along(subsets_list), function(i) {  
  p <- ggplot(subsets_list[[i]], aes(x = zipcode_offenders_per_resident, y = arrested, color = factor(black_not_white))) +
    geom_point(alpha = 4/10, position = position_jitter(height = 0.1)) + 
    stat_smooth() + xlab("Zip Codes' Offenders per Resident") + ylab("Arrested") +
    xlim(0, 0.35) + theme_bw() + ggtitle(names(subsets_list)[[i]]) +
...
raw_plots_grid <- grid_arrange_shared_legend(raw_plots_list[[1]], raw_plots_list[[2]],
                                             ...
                                             raw_plots_list[[9]])
ggsave("offenders_per_resident_raw_arrested.png", plot = raw_plots_grid,
       dpi = 600, width = 12, height = 10)
