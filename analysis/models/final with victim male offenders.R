...
models_wvictims <- lapply(subsets, function(subset) {
  model1 <- glmer(arrested ~ black_not_white + (1 + black_not_white | ori),
  ...
  model2 <- glmer(arrested ~ black_not_white + w_officers_percent_standardized +
                    ...
                    male_officers_percent_standardized + agency_bdgt_per_offender_standardized + com_bt + 
                    tech_typ_vpat + hir_trn_no_p + min_hiring_educ_gths +
                    (1 + black_not_white | ori),
                  data = subset, family = binomial(link = "logit"))
  model3 <- glmer(arrested ~ black_not_white * w_officers_percent_standardized +
                    victim_analyzing_sex + victim_analyzing_interracial +
                    victim_analyzing_known + offense_in_residence1 +
                    male_officers_percent_standardized + agency_bdgt_per_offender_standardized + com_bt + 
                    tech_typ_vpat + hir_trn_no_p + min_hiring_educ_gths +
...
htmlreg(models_list_wvictims, center = F, file = 'with_victims_indiv_male.doc', digits = 3,
        ...
        custom.coef.names = c("Constant", "Black (ref = White)",
...
# Models 2 (simple assault), 8 (intimidation), and 11, 12 (robbery) do not converge (max relgrad > 0.001)
...
htmlreg(coef_models_wvictims, file = 'with_victims_coef_male.doc', digits = 3,
