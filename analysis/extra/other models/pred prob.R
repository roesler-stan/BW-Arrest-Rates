...
simple_assault_data$w_officers_percent_standardized_cat <- cut(simple_assault_data$w_officers_percent_standardized, seq(-2.1, 0.65, 0.05))

# Choose non-missing data for which to find the predicted values
officers_data <- na.omit(simple_assault_data[, c("arrested", "black_not_white", "w_officers_percent_standardized", 
                                  ...
                                  "w_officers_percent_standardized_cat")])

# without "response", predict gives log odds
predicted1 <- predict(m3_simple_assault, type = "response")
predicted1_ori <- data.frame(officers_data, predicted1)
