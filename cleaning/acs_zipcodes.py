""" Make acs_zipcodes_13.csv, zipcode-level ACS demographic data """
...
"""
Data sources:
    5-digit zip code tabulation areas (860)

    race: 2013 5-year zipcode-level file for B02001, ACS_13_5YR_B02001_with_ann.csv
    gender and age: 2013 5-year zipcode-level file for S0101, ACS_13_5YR_S0101_with_ann.csv
    gender and age for white, black people: 2013 5-year zipcode-level file for B01001A and B01001B, ACS_14_5YR_B01001A_with_ann.csv, ACS_14_5YR_B01001B_with_ann.csv
    mean income: 2013 5-year zipcode-level file for S1902, ACS_13_5YR_S1902_with_ann.csv
...
def main():
    ...
    outfile = '../../Data/acs_zipcodes_13.csv'
    ...
    full_data = pandas.merge(full_data, gender_age_data, how = 'outer', on = 'zipcode')
    full_data = pandas.merge(full_data, gender_race_age_data, how = 'outer', on = 'zipcode')
    full_data = pandas.merge(full_data, inc_data, how = 'outer', on = 'zipcode')
    full_data = pandas.merge(full_data, race_inc_data, how = 'outer', on = 'zipcode')
    full_data = pandas.merge(full_data, emp_data, how = 'outer', on = 'zipcode')
    
    full_data['zipcode'] = full_data['zipcode'].map(remove_text)
...
def clean_race(race_filename):
    ...
    dataset = dataset.drop(dataset[['Id', 'Id2']], axis = 1)

    dataset = dataset.rename(columns = lambda x: x.replace('Estimate; Total: - ', ''))
    dataset = dataset.rename(columns = lambda x: x.split(' alone')[0].lower())
   
    dataset = dataset.rename(columns = {'geography': 'zipcode', 'white':'w_residents', 'black or african american':'b_residents',
...
def gender_race_age(filenames):
    ...
    for i, filename in enumerate(filenames):
        ...
        desired_cols = ['Geography', 'Estimate; Total:', 'Estimate; Male:',
                        'Estimate; Male: - 18 and 19 years', 'Estimate; Male: - 20 to 24 years']
        dataset = dataset[desired_cols]
        
        dataset = dataset.rename(columns = {'Geography': 'zipcode'})
        dataset = dataset.rename(columns = lambda x: re.sub('Estimate; ','', x))
        ...
        dataset = dataset.rename(columns = {race + '_zipcode': 'zipcode'})
        dataset = dataset.rename(columns = lambda x: x.lower())
        ...
        if full_data.empty:
            ...
            full_data = pandas.merge(full_data, dataset, how = 'outer', on = 'zipcode')
...
def clean_inc(filename):
    ...
    rename_dict = {'Geography': 'zipcode', 'Total; Estimate; All households':'total_inc_households',
...
def race_inc(black_file, white_file):   
    ...
    def rename_inc(dataset):
        rename_dict = {'Geography':'zipcode', 'Estimate; Total:':'inc_households',
    ...
    full_data = full_data.rename(columns = {'bzipcode': 'zipcode'})

    white_data = white_data.rename(columns = lambda x: 'w' + x)
    white_data = white_data.rename(columns = {'wzipcode': 'zipcode'})

    full_data = pandas.merge(full_data, white_data, how = 'outer', on = 'zipcode')
...
def emp(filename):
    ...
    desired_cols = ['Geography', total_name, unemp_name, w_unemp, b_unemp]
    dataset = dataset[desired_cols]
    dataset = dataset.rename(columns = {'Geography': 'zipcode', total_name: 'total_unemp',
