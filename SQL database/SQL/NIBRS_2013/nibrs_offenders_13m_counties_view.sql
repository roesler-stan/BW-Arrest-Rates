--nibrs_offenders_13m_counties_view

CREATE VIEW nibrs_offenders_13m_counties_view AS
(SELECT * FROM
((SELECT
	ct.fips_county1_no offenders_county_no,
	...
	FROM nibrs_offenders_13m as ct
	WHERE ct.race IS NOT NULL and ct.fips_county1_no IS NOT NULL
	GROUP BY ct.fips_county1_no) all_offenders
LEFT OUTER JOIN
(SELECT
	w_ct.fips_county1_no w_offenders_county_no,
	...
	FROM nibrs_offenders_13m as w_ct
	WHERE w_ct.race = 'white' and w_ct.fips_county1_no IS NOT NULL
	GROUP BY w_ct.fips_county1_no) w_offenders
ON all_offenders.offenders_county_no = w_offenders.w_offenders_county_no) all_and_white
LEFT OUTER JOIN
(SELECT
	b_ct.fips_county1_no b_offenders_county_no,
	...
	FROM nibrs_offenders_13m as b_ct
	WHERE b_ct.race = 'black' and b_ct.fips_county1_no IS NOT NULL
	GROUP BY b_ct.fips_county1_no) b_offenders
